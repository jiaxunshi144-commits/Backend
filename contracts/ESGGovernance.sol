...
/**
 * ESGGovernance (Full, simplified constructor)
...
contract ESGGovernance {
    ...
    mapping(bytes32 => address) private _uploader;
    ...
    event ReportAttested(bytes32 indexed contentHash, address indexed auditor, bool passed, string reason, uint256 timestamp);
    event AttestationRevoked(bytes32 indexed contentHash, address indexed auditor, string reason, uint256 timestamp);

    // ---------------------- 激励机制 ----------------------
    IGreenToken public rewardToken;
    ...
    function attestReport(bytes32 contentHash, bool passed, string calldata reason) external onlyAuditor {
    ...
    function revokeAttestation(bytes32 contentHash, string calldata reason) external onlyAuditor {
    ...
    function getReportStatus(bytes32 contentHash) external view returns (
        bool registered,
        bool hasAttestation,
        bool passed,
        address auditor,
    ...
    function getAttestationHistory(bytes32 contentHash) external view returns (AttestationHistory[] memory) {
